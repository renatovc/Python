...
# Definimos una definición auxiliar para la Definición 1 solicitada
...
# Se empieza la Definición 1 solicitada
def validar_posicion(pos, tablero):
  # Definimos columnas y filas dentro del tablero con la posición dada
  columnas_val = "ABCDEFGH"
  columna, fila = posicion_en_tablero(pos)

  # Verifico que la posición sea valida
  if columna not in columnas_val or fila < 1 or fila > 8:
    return -1

  # Definimos filas y columnas en el tablero
  fila_in = 8 - fila
  columna_in = columnas_val.index(columna)

  # Verificamos que si la posicion en tablero esta ocupada
...
# Creamos el tablero para verificar las posiciones e incluirlas en la impresión
def entregar_tablero(piezas):
  ...
  # Separamos los elementos que nos estreguen
  elementos = piezas.split(",")

  # Defino a que cosa corresponde cada dato entregado
  for elemento in elementos:
    if len(elemento) == 4:
      ...
      # Verifico que la posición este dentro de estos valores
      if color in ("B", "N") and pieza in (
          'P', 'T', 'C', 'A', 'D',
          'R') and columna in ('A', 'B', 'C', 'D', 'E', 'F', 'G',
                               'H') and fila in ('1', '2', '3', '4', '5', '6',
                                                 '7', '8'):
        fila_ind = 8 - int(fila)
        columna_ind = ord(columna) - ord("A")
        pieza_y_color = color + pieza

        # Verificamos que si el espacio en tablero esta disponible le imprima la posicion y pieza correspondiente
...
def es_valido(tablero):
  # Generamos contadores de piezas en el tablero
  cont_blancas_p = 0
  cont_blancas_t = 0
  cont_blancas_c = 0
  cont_blancas_a = 0
  ...
  for fila_ind in range(8):
    for columna_ind in range(8):
      # Defino la variable pieza para hacer mas sencillo el buscar las piezas
      pieza = tablero[fila_ind][columna_ind]

      # Empiezo el ciclo de busqueda y de sumar al contador
      if pieza != "  ":
        jugador = pieza[0]
        tipo_pieza = pieza[1]

        # Contabilizo las piezas Blancas
        if jugador == "B":
          if tipo_pieza == "P":
            cont_blancas_p += 1
          elif tipo_pieza == "T":
          ...
          # Verifico la posicion y si es peon negro o blanco
  ...
  condiciones_alfiles_fila = (alfil_blanco_blanco and alfil_blanco_negra
                              and alfil_negro_negro and alfil_negro_blanco)
...
def rey_en_jaque(posicion_rey, oponente, tablero):
  for fila_ind in range(8):
    for columna_ind in range(8):
      pieza = tablero[fila_ind][columna_ind]
      if pieza[0] == oponente and es_movimiento_valido(
          pieza, (fila_ind, columna_ind), posicion_rey):
        return True
...
def es_movimiento_peon(color, origen, destino):
  ...
  if color == "B":
    return fila_destino == fila_origen - 1 and abs(columna_destino -
                                                   columna_destino) == 1
  else:
    return fila_destino == fila_origen + 1 and abs(columna_destino -
                                                   columna_origen) == 1
...
def es_movimiento_caballo(origen, destino):
  ...
  return (abs(fila_destino - fila_origen) == 2
          and abs(columna_destino - columna_origen)
          == 1) or (abs(columna_destino - columna_origen) == 2
                    and abs(fila_destino - fila_origen) == 1)
...
def es_movimiento_alfil(origen, destino):
  ...
  return abs(fila_destino - fila_origen) == abs(columna_destino -
                                                columna_origen)

# Definición para el movimiento de la reina
def es_movimiento_dama(origen, destino):
  return es_movimiento_torre(origen, destino) or es_movimiento_alfil(
      origen, destino)
...
def es_movimiento_rey(origen, destino):
  ...
  return abs(fila_destino - fila_origen) <= 1 and abs(columna_destino -
                                                      columna_origen) <= 1
