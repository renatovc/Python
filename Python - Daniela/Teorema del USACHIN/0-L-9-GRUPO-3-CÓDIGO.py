...
# SECCIÓN DEL CURSO: 0-L-9
# PROFESOR DE TEORÍA: FELIPE FUENTES BRAVO / GABRIEL GODOY / MATÍAS TOBAR
# PROFESOR DE LABORATORIO: JUAN GONZALEZ REYES
# GRUPO: 3
# INTEGRANTES
# 1. Alexander Aguayo Cortés 21.564.899-0
# 2. Anita Godoy Araneda 21.585.469-8
# 3. Daniela Castro Añasco 21.190.036-9
# 4. Javier Umanzor Umanzor 21.404.601-6
# 5. Mía Castro López 21.565.080-4
...
keyBot = "MTI3MTgxMjk4NDgxMzEyNTY2NA.GSECVZ.N9PsCxcd2A4wstKHBMxPXi2rv_ApYwrga6jQ_0"
...
async def informacion(ctx):  # Define un comando asincrónico llamado 'informacion'.
    ...
    if moderadores:
        for moderador in moderadores:
            if moderadores_value:
                moderadores_value += ", "
            moderadores_value += moderador
    else:
        moderadores_value = "No hay moderadores"
        
    embed.add_field(name="Moderadores", value=moderadores_value, inline=False)
...
async def on_message(message):
    ...
    improperio_encontrado = False
    for improperio in improperios:
        if improperio in mensaje:
            improperio_encontrado = True
    ...
    for link in links_prohibidos:
        if re.search(link, mensaje):
...
async def promedio(ctx):
    # ENTRADA: Usuario ejecuta el comando /promedio

    # SALIDA INICIAL: Instrucciones al usuario
    instrucciones = ("Se debe ingresar cada nota en una escala de 1.0 a 7.0, o en su defecto, si esa nota te falta un 0 (cero). Se espera recibir un total de 6 notas y 3 porcentajes.\n")
    await ctx.send(instrucciones)
    
    # Preguntas que el bot hará para obtener las notas y porcentajes
    preguntas = [
        "Ingresa la nota de tu PEP 1: ",
        "Ingresa la nota de tu PEP 2: ",
        "¿Cuánto vale el porcentaje del promedio de las PEPs? ",
        "Ingresa la nota de tu Control 1: ",
        "Ingresa la nota de tu Control 2: ",
        "¿Cuánto vale el porcentaje del promedio de los controles? ",
        "Ingresa la nota de tu Taller 1: ",
        "Ingresa la nota de tu Taller 2: ",
        "¿Cuánto vale el porcentaje del promedio de los talleres? "
    ]
    
    # Validación para asegurar que el mensaje proviene del usuario que ejecutó el comando
    def check(m):
        return m.author == ctx.author and m.channel == ctx.channel
    
    # PROCESO:
    # Recolección de respuestas del usuario
    respuestas = []
    for pregunta in preguntas:
        await ctx.send(pregunta)
        try:
            respuesta = await bot.wait_for('message', timeout=60.0, check=check)
            respuestas.append(respuesta.content)
        except TimeoutError:
            await ctx.send('Se acabó el tiempo para responder. Intenta de nuevo.')
            return
    
    # Validación y conversión de entradas del usuario
    try:
        nota_pep_1 = float(respuestas[0])
        nota_pep_2 = float(respuestas[1])
        porcentaje_pep = int(respuestas[2])
        nota_control_1 = float(respuestas[3])
        nota_control_2 = float(respuestas[4])
        porcentaje_control = int(respuestas[5])
        nota_taller_1 = float(respuestas[6])
        nota_taller_2 = float(respuestas[7])
        porcentaje_taller = int(respuestas[8])
    except ValueError:
        await ctx.send('Por favor, asegúrate de ingresar valores válidos.')  # Salida: Mensaje de error por valores inválidos
        return
    
    # Constantes para el cálculo de notas
    nota_maxima = 7
    nota_aprobado = 3.95

    # Convertir porcentajes a decimales
    porcentaje_pep = porcentaje_pep / 200
    porcentaje_control = porcentaje_control / 200
    porcentaje_taller = porcentaje_taller / 200

    # Calcular el promedio final ponderado
    promedio_final = (nota_pep_1 * porcentaje_pep + nota_pep_2 * porcentaje_pep + nota_control_1 * porcentaje_control + nota_control_2 * porcentaje_control + nota_taller_1 * porcentaje_taller + nota_taller_2 * porcentaje_taller)


    # Verificar si el usuario aprobó
    if promedio_final >= nota_aprobado:
        await ctx.send(f"¡Felicitaciones! (～￣▽￣)～ Ya estás aprobado, tu promedio es {promedio_final:.1f}!")
        return

    # Función auxiliar para calcular la nota faltante
    def calcular_nota_faltante(promedio_actual, porcentaje):
        h = 0
        while h < nota_maxima:
            promedio_falta = promedio_actual + h * porcentaje 
            if promedio_falta >= nota_aprobado:
                return round(h, 1)
            h = h+0.1
        return None

    # Calcular notas faltantes si el usuario no ha aprobado
    mensaje = ""

    if nota_pep_1 == 0:
        falta = calcular_nota_faltante(promedio_final, porcentaje_pep)
        if falta is not None:
            mensaje += f"Necesitas una nota de {falta} en la PEP 1 para pasar. Tu puedes! ;)\n"

    if nota_pep_2 == 0:
        falta = calcular_nota_faltante(promedio_final, porcentaje_pep)
        if falta is not None:
            mensaje += f"Necesitas una nota de {falta} en la PEP 2 para pasar. Tu puedes! ;)\n"

    if nota_control_1 == 0:
        falta = calcular_nota_faltante(promedio_final, porcentaje_control)
        if falta is not None:
            mensaje += f"Necesitas una nota de {falta} en el Control 1 para pasar. Tu puedes! ;)\n"

    if nota_control_2 == 0:
        falta = calcular_nota_faltante(promedio_final, porcentaje_control)
        if falta is not None:
            mensaje += f"Necesitas una nota de {falta} en el Control 2 para pasar. Tu puedes! ;)\n"

    if nota_taller_1 == 0:
        falta = calcular_nota_faltante(promedio_final, porcentaje_taller)
        if falta is not None:
            mensaje += f"Necesitas una nota de {falta} en el Taller 1 para pasar. Tu puedes! ;)\n"

    if nota_taller_2 == 0:
        falta = calcular_nota_faltante(promedio_final, porcentaje_taller)
        if falta is not None:
            mensaje += f"Necesitas una nota de {falta} en el Taller 2 para pasar. Tu puedes! ;)\n"

    if mensaje != "":
        await ctx.send(mensaje)
    else:
        await ctx.send("Ya no hay nada que hacer... ya reprobaste ＞﹏＜. Tu promedio es: {promedio_final:.1f}!")
## End: comando promedio ##

## Start: comando profesores ##
@bot.command()
