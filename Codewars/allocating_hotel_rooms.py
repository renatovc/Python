...
def allocate_rooms(customers):
    sorted_customers = sorted(enumerate(customers), key=lambda x: (x[1][0], x[1][1]))
    
    heap = []
    room_allocations = [0] * len(customers)
    room_mapping = {}
    room_count = 0
    
    for index, (arrival, departure) in sorted_customers:
        if heap and heap[0][0] < arrival:
            earliest_departure, room_number = heapq.heappop(heap)
        else:
            room_count += 1
            room_number = room_count
        
        room_allocations[index] = room_number
        heapq.heappush(heap, (departure, room_number))
